...
function fetchAnimeData() {
  ...
  return fetch('https://graphql.anilist.co', {
    ...
    .then((json) => {
      ...
      if (!media.length) return FALLBACK_ANIME;
...
function App() {
  ...
  const years = useMemo(() => {
    const unique = [...new Set(animeList.map((item) => item.year).filter(Boolean))];
  ...
  return (
    <header>
      <nav className="nav-bar" aria-label="Main navigation">
        <Link className="brand" to="#/">Anime Library</Link>
        <ul>
          <li>
            <Link to="#/">Home</Link>
          </li>
...
function HomePage({ bannerAnime, loading, error, searchTerm, onSearch, sortKey, onSort, viewMode, onViewMode, yearFilter, onYearFilter, years, page, pageCount, onPage, pageItems, favorites, onToggleFavorite, totalCount }) {
  return (
    <>
      <section className="hero-banner" style={{ backgroundImage: `url(${bannerAnime.img})` }}>
        ...
        <div className="hero-copy">
          ...
          <div className="hero-buttons">
            <Link to={`#/detail/${bannerAnime.id}`} className="btn-primary">View Details</Link>
      ...
      {loading ? (
        <p style={{ padding: '1rem 0', color: 'var(--muted)' }}>Loading list...</p>
      ) : error ? (
        <p style={{ padding: '1rem 0', color: 'var(--muted)' }}>{error}</p>
      ) : pageItems.length === 0 ? (
        <p style={{ padding: '1rem 0', color: 'var(--muted)' }}>No anime found. Try adjusting your search or filters.</p>
      ) : (
